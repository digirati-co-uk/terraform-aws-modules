output "task_definition_arn" {
  ...
  value       = length(var.mount_points_main) == 0 && length(var.mount_points_sidecar_1) == 0 && length(var.mount_points_sidecar_2) == 0 && length(var.mount_points_sidecar_3) == 0 ? join("", aws_ecs_task_definition.task.*.arn) : ""
}
